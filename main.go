...
//
// Build: go build -o folio.exe ./cmd/folio
// Cross-compile for Windows: GOOS=windows GOARCH=amd64 go build -o folio.exe ./cmd/folio
// Cross-compile for macOS:   GOOS=darwin  GOARCH=arm64 go build -o folio     ./cmd/folio
// Cross-compile for Linux:   GOOS=linux   GOARCH=amd64 go build -o folio     ./cmd/folio
//
// Zero runtime dependencies. Pandoc (pandoc.org) must be installed separately.
// Single binary deployment: copy folio.exe to any Windows machine, run immediately.
package main
...
import (
	...
	"github.com/folioprotocol/folio-go/internal/fingerprint"
	"github.com/folioprotocol/folio-go/internal/transport"
	"github.com/folioprotocol/folio-go/internal/validate"
)
...
// folio track <document> [--author NAME] [--title TITLE] [--note TEXT]
...
// folio save <document> [--author NAME] [--note TEXT]
...
func cmdHistory(args []string) int {
	...
	// Sign-offs with stale detection
...
// folio verify <document> [--version N]
func cmdVerify(args []string) int {
	...
	return 2 // distinct exit code: clean run, mismatch result
}

// folio status <document>
...
// folio convert <input> <output> [--author NAME]
func cmdConvert(args []string) int {
	...
	// Write record to the new format
...
// folio milestone <document> --label TEXT [--author NAME]
...
func cmdRedline(args []string) int {
	...
	// Convert both to plain text via pandoc and diff
	...
	// Line-level diff of plain text representations
...
func cmdVersion() int {
	...
	fmt.Printf("Protocol: https://github.com/folioprotocol/spec\n")
...
func printLineDiff(text1, text2 string) {
	...
	// Very simple LCS-based diff — production should use a proper Myers diff
	added, removed := 0, 0
	maxLen := len(lines1)
...
func defaultAuthor() string {
	...
	if v := os.Getenv("USER"); v != "" { // macOS/Linux
		return v
	}
	return "unknown"
}
...
func flagInt(args []string, flag, defaultVal int) int {
